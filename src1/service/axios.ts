...
export default {
  get: (url: string, config?: AxiosRequestConfig) => observableRequest(ax.get(url, config)),
  post: (url: string, data?: any, config?: AxiosRequestConfig) => observableRequest(ax.post(url, data, config))
};
